...
def app():
    ...
    if st.button("Predecir tendencia"):
        ...
        plt.title('Decision Tree Predictions vs Actual Trends')
        plt.xlabel('Date')
        plt.ylabel('Trend')
        plt.legend()
        st.pyplot(plt)
        ...
        plt.title('Comparación de Tendencias Predichas vs Reales')
        plt.xlabel('Fecha')
        ...
        st.write("### PREDICCIÓN PARA EL SIGUIENTE DIA")
        last_price = scaler.transform([[X.iloc[-1]['Lag_1']]])
        next_day_prediction = tree_model.predict(last_price)
