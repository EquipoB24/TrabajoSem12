...
# Clase personalizada para actualizar la barra de progreso durante el entrenamiento
class StreamlitProgressBar(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        self.epoch_text.text(f"Epoch {epoch + 1}/{self.epochs}")
...
def app():
    ...
    if st.button("Predecir tendencia"):
        ...
        epochs = 20
        progress_bar = StreamlitProgressBar(epochs)

        # Entrenar el modelo con la barra de progreso
        model.fit(X_train, y_train, batch_size=1, epochs=epochs, callbacks=[progress_bar])
