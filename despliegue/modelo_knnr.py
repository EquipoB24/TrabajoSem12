...
def app():
    st.title("Predicción del precio de acciones con KNN")
    ...
    if st.button("Predecir precio"):
        ...
        # Crear y entrenar el modelo KNN Regression
        ...
        next_day_prediction = knn_model.predict(last_price)
        next_day_prediction = scaler.inverse_transform(next_day_prediction.reshape(-1, 1))
        st.write(f"El precio de BHP se pronóstica según el modelo KNN para el siguiente día como: ${next_day_prediction[0][0]:.2f} por acción.")
        ...
        plt.title('Métricas de Evaluación del Modelo KNN')
        st.pyplot(plt)